...
export class AuthService {
  ...
  login(u:UserLogin){
    ...
    this.http.post<{accessToken:string, user:any}>('http://localhost:3000/login', u).subscribe((res)=>{
  ...
  signup(u:UserSignup) {
    this.http.post<{accessToken:string, user:any}>('http://localhost:3000/signup', u).subscribe((res)=>{
